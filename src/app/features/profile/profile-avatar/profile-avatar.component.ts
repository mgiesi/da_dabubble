import { Component, effect, inject, Injector, Input, input, InputSignal, OnDestroy, signal, Signal } from '@angular/core';
...
export class ProfileAvatarComponent implements OnDestroy {
  ...
  private async _load(imgUrl: string | null, fallback: string) {
    ...
    const { src, revoke } = await this.imgStorage.resolveImgSrc(imgUrl ?? '', this.fallbackUrl());
